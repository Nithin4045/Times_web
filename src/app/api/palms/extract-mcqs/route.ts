...
export async function POST(req: NextRequest) {
  ...
  try {
    const fileJob = await prisma.generate_jobs.create({
      data: {
        ...
        request_data: JSON.stringify({ fileBase64, fileName, file_type }),
        response_data: '',
        status: 'started',
        api_endpoint: '/api/palms/extract-mcqs',
        percentage: 0,
      },
    });
    job_id = fileJob.id;
    console.log('[extract-mcqs] Created generate_jobs with id:', job_id);
  } catch (e) {
    console.error('[extract-mcqs] Failed to create generate_jobs:', e);
  }
  ...
  if (job_id) backendForm.append('job_id', job_id.toString());

  // Call FastAPI backend
  console.log('[extract-mcqs] Starting backend streaming request...');
  const backendRes = await fetch(process.env.PYTHON_PALMS_URL + '/extract-mcqs/stream', {
    method: 'POST',
    body: backendForm,
  });
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      let buffer = '';
      console.log('[extract-mcqs] Streaming from backend...');
      while (true) {
        ...
        for (const part of parts) {
          if (part.startsWith('data:')) {
            try {
              ...
              console.log('[extract-mcqs] Received chunk:', json);
              if (json.error) {
                error = json.error;
                ...
                if (job_id) {
                  try {
                    ...
                    console.error('[extract-mcqs] Failed to update generate_jobs with error:', e);
                  }
                }
                controller.enqueue(encoder.encode(`data: ${JSON.stringify(json)}\n\n`));
                controller.close();
                console.log('[extract-mcqs] Streaming ended with error.');
                return;
              }
              ...
              if (job_id && typeof percent === 'number') {
                try {
                  ...
                  console.error('[extract-mcqs] Failed to update generate_jobs percentage:', e);
                }
              }
              if (json.data) questions.push(json.data);
              controller.enqueue(encoder.encode(`data: ${JSON.stringify(json)}\n\n`));
              console.log('[extract-mcqs] Streamed to client:', json);
            } catch (e) {
      ...
      controller.close();
      console.log('[extract-mcqs] Streaming finished.');
    },
  });
